...
def evaluate_tool_redundancy_llm_judge(
    actual_tool_calls: list[dict[str, Any]],
) -> Evaluation:
    """
    Evaluates whether tool calls are redundant using LLM as judge.

    Assesses if the agent is making unnecessary duplicate tool calls
    or if each tool call serves a unique purpose.

    Score: 1.0 = no redundancy (good), 0.0 = highly redundant (bad)
    """
    from deepeval.metrics import GEval
    from deepeval.test_case import LLMTestCaseParams
    ...
    if not actual_tool_calls:
        return Evaluation(
            name="tool_redundancy_llm_judge",
            value=1.0,
            comment="No tool calls to evaluate for redundancy",
        )

    # Format tool calls for evaluation
    tool_calls_text = "\n".join(
        f"{i+1}. {tc['name']}({tc.get('args', {})})"
        for i, tc in enumerate(actual_tool_calls)
    )

    # Create GEval metric for redundancy detection
    redundancy_metric = GEval(
        name="Tool Redundancy",
        evaluation_params=[LLMTestCaseParams.ACTUAL_OUTPUT],
        evaluation_steps=[
            "Identify if any tool calls are making duplicate or near-duplicate queries",
            "Check if tool calls are fetching the same information repeatedly",
            "Assess if each tool call serves a unique and necessary purpose",
            "Evaluate if the tool calls could be consolidated or optimized",
            "Score 1.0 if no redundancy (all calls are necessary and unique), score 0.0 if highly redundant",
        ],
        threshold=0.5,
    )

    test_case = LLMTestCase(
        input="",  # Not used for this evaluation
        actual_output=f"Tool Calls Made:\n{tool_calls_text}",
    )

    redundancy_metric.measure(test_case)

    # Invert score: GEval gives high score for "good" redundancy detection
    # We want high score for "no redundancy"
    # The evaluation_steps already instruct to score 1.0 for no redundancy
    score = redundancy_metric.score

    return Evaluation(
        name="tool_redundancy_llm_judge",
        value=score,
        comment=redundancy_metric.reason or "Tool redundancy evaluated by LLM judge",
    )
